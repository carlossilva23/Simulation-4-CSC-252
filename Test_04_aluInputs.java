public class Test_04_aluInputs {
	public static void main(String[] args) {
		...
		state.instMemory[0] = sim4_test_commonCode.ADD(sim4_test_commonCode.S_REG(0), sim4_test_commonCode.S_REG(1),
				sim4_test_commonCode.S_REG(2));
		state.instMemory[1] = sim4_test_commonCode.SUB(sim4_test_commonCode.S_REG(3), sim4_test_commonCode.S_REG(4),
				sim4_test_commonCode.S_REG(5));
		state.instMemory[2] = sim4_test_commonCode.ADDI(sim4_test_commonCode.T_REG(2), sim4_test_commonCode.T_REG(3),
				-1);
		state.instMemory[3] = sim4_test_commonCode.ADDI(sim4_test_commonCode.T_REG(4), sim4_test_commonCode.T_REG(5),
				16);
		...
		// state.instMemory[10] = sim4_test_commonCode.ORI(18, 19, 0x9abc); // Commented
		// out in original
		...
		/*
		 * fill in all of the registers, so that we can figure out if we're reading the
		 * right values.
		 */
		for (int i = 0; i < 32; i++)
		...
		for (state.pc = 0x0000; state.pc < 0x1000; state.pc += 4) {
			...
			if (inst != 0) {
				...
				if (ok == 0) {
					System.out.printf(
							"execute_CPUControl() returned 0 with opcode=0x%02x funct=0x%02x at PC 0x%04x_%04x\n",
							fields.opcode, fields.funct, (state.pc >> 16) & 0xffff, state.pc & 0xffff);
					continue;
				}
				...
				System.out.printf("PC: 0x%04x_%04x   Instruction: 0x%04x_%04x\n", (state.pc >> 16) & 0xFFFF,
						state.pc & 0xFFFF, (inst >> 16) & 0xFFFF, inst & 0xFFFF);
			...
			if (!java.util.Arrays.equals(state.instMemory, stateOrig.instMemory)
					|| !java.util.Arrays.equals(state.dataMemory, stateOrig.dataMemory) || state.pc != stateOrig.pc
